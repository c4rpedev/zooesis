    ...
    const EditableHemogramTable = ({ parameters, values, onValueChange, analysisType }) => {
      ...
      const getDisplayValue = (paramId) => {
        const paramValue = values[paramId];
        if (typeof paramValue === 'object' && paramValue !== null && paramValue.hasOwnProperty('value')) {
                  ...
                  >
                    ...
                    <TableCell className="px-4 py-3">
                      <Input
                        id={param.id}
                        type="text"
                        value={getDisplayValue(param.id)}
                    ...
                    <TableCell className="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{param.unit || 'N/A'}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">
                      {param.refMin && param.refMax ? `${param.refMin} - ${param.refMax}` : (param.reference_range || 'N/A')}
                    </TableCell>
                  </motion.tr>
                ))}
              </TableBody>
